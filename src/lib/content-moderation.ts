...
export async function assertGenerationContentIsSafe({
  ...
  if (!apiKey) {
    throw new ContentModerationUnavailableError('Content protection is not configured.');
  }
  ...
  try {
    ...
    throw new ContentModerationUnavailableError('Content protection could not be reached. Please try again shortly.');
  }

...
const requestSheet = async (prompt: string) => {
  ...
  if (!response.ok || !payload.data?.[0]?.b64_json) {
    throw new Error(
      payload.error?.message ?? "The image generation service could not create a preview sheet.",
    );
  }
  ...
  await Promise.all(
    SpriteTypeViews[spriteType].map(async (view) => {
      ...
      const outputPath = path.join(
        OUTPUT_DIRECTORY,
        `${TYPE_SLUGS[spriteType]}-${VIEW_SLUGS[view]}.png`,
      );
...
const main = async () => {
  ...
  const spriteTypes = REQUESTED_TYPE
    ? Object.values(SpriteType).filter(
        (spriteType) => spriteType.toLowerCase() === REQUESTED_TYPE,
      )
  ...
  for (const [index, spriteType] of spriteTypes.entries()) {
    if (!(await typeNeedsGeneration(spriteType))) {
      console.log(
        `[${index + 1}/${spriteTypes.length}] keeping ${TYPE_SLUGS[spriteType]} sheet previews`,
      );
      continue;
    }

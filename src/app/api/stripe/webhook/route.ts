...
import { BILLING_PRODUCTS, isBillingProductId, type BillingProductId } from '@/lib/billing-products';
...
const getId = (value: string | { id: string } | null) => (typeof value === 'string' ? value : (value?.id ?? null));
...
const getUserEmail = (metadata: Stripe.Metadata | null | undefined) => metadata?.user_email ?? null;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (event.type === 'invoice.paid') {
      ...
      if (invoice.billing_reason === 'subscription_cycle' && productId && userEmail) {
        await grantProductCredits({
